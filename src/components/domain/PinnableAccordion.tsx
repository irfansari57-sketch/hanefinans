...
export function PinnableAccordion({
  ...
  return (
    <details
      className={cn(
        'mb-3 overflow-hidden rounded-xl border bg-bg-soft transition',
        pinned ? 'border-warning/30' : 'border-border',
      )}
      open={open}
      onToggle={(e) => setOpen((e.currentTarget as HTMLDetailsElement).open)}
    >
      <summary className="flex cursor-pointer items-center gap-3 px-4 py-3 select-none [&::-webkit-details-marker]:hidden hover:bg-bg-card/30">
        {icon && (
          <span className={cn('grid h-9 w-9 shrink-0 place-items-center rounded-lg', iconColorClass)}>
            {icon}
          </span>
        )}
