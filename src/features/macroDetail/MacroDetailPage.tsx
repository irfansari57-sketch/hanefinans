...
/**
 * Global Piyasa endeksi / emtia / volatilite / makro göstergesi detay sayfası.
 * URL: /macro/<yahoo-symbol>  (ör. /macro/%5ETNX  ABD 10Y için)
 *
 * Aynı GlobalPage.tsx'te listelenen tüm semboller burada açılabilir.
 */
interface MacroMeta {
  label: string;
  category: 'ABD Endeksi' | 'Avrupa Endeksi' | 'Asya Endeksi' | 'Emtia' | 'Risk & Volatilite' | 'Faiz';
  unit?: string;
  description: string;
}
    ...
    <>
      ...
      <div className="card relative mb-4 overflow-hidden p-6">
        <div className={cn('pointer-events-none absolute -right-20 -top-20 h-72 w-72 rounded-full blur-3xl',
          ...
          meta.category === 'Risk & Volatilite' || meta.category === 'Faiz' ? 'bg-danger/10' :
          'bg-accent/10')} />
